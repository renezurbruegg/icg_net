...
class ConvolutionalOccupancyNetworkContact(nn.Module):
    ''' Occupancy Network class.
    ...
    def grad_refine(self, x, pos, bound_value=0.0125, lr=1e-6, num_step=1):
        ...
        with torch.no_grad():
            #print(pos, pos_tmp)
            pos_tmp = torch.maximum(torch.minimum(pos_tmp, u_bound), l_bound)
            qual_out, width_out = self.forward(x, pos_tmp)
            # print(pos, pos_tmp, qual_out)
            # print(qual_out.mean().item())
        # import pdb; pdb.set_trace()
        # self.train()
        for p in self.parameters():
            p.requres_grad = True
        # import pdb; pdb.set_trace()
        return qual_out, pos_tmp, width_out
