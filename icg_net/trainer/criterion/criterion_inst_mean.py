...
class SetCriterion(nn.Module):
    ...
    def loss_sdf(self, outputs, targets, indices, num_masks, mask_type):
        ...
        if self.sdf_loss_type == "l1":
            ...
            loss = 0.1*torch.stack([dice_loss(p, (t >= 0).float(), 1) for p, t in zip(pred_sdf, target_sdf)]).mean()
            loss += 0.9*torch.stack(
                [sigmoid_ce_loss_jit(p, (t >= 0).float(), 1) for p, t in zip(pred_sdf, target_sdf)]
            ).mean()
...
def grasp_loss_with_width(pred, tgt, loss_type = "bce"):
    bce_labels = tgt[:, :-1]
    bce_preds = pred[:, :-1]
    bce_loss = F.binary_cross_entropy_with_logits(bce_preds, (bce_labels > 0).float(), reduction = 'none').mean(-1).ravel()
    
    if "both" == loss_type:
        bce_loss = bce_loss * 0.9 + 0.1 * soft_dice_loss(bce_preds, (bce_labels > 0).float(), 1)

    width_labels = tgt[:, -1]
    width_preds = pred[:, -1]
    width_loss = (F.mse_loss(40 * width_preds, 40 * width_labels,  reduction = 'none') * (bce_labels > 0).any(dim=-1)).ravel()
    ...
    def loss_affordance(self, outputs, targets, indices, num_masks, mask_type):
        ...
        if "scene_grasps" in outputs:
            ...
            if len(scene_loss_width) > 0:
                loss["loss_grasp_scene"] = torch.cat(scene_loss_bce).mean()
                loss["loss_grasp_width_scene"] = torch.cat(scene_loss_width).mean()
        ...
        if "object_grasps" in outputs:
            ...
            if len(object_loss) > 0:
                loss["loss_grasp_object"] = torch.cat(object_loss).mean()
                loss["loss_grasp_width_object"] = torch.cat(object_loss_width).mean()
