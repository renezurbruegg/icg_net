...
class SemanticSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.dataset_name == "stpls3d":
            ...
            # print("returning", coordinates, features, labels, self.data[idx]['raw_filepath'].split("/")[-1], \
            #          raw_color, raw_normals, raw_coordinates, idx)
            return coordinates, features, labels, self.data[idx]['raw_filepath'].split("/")[-1].replace(".npz", ""), \
                   raw_color, raw_normals, raw_coordinates, idx
        else:
