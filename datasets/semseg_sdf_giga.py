...
def relabel_from_zero(labels: np.ndarray):
    "Relabels an array of labels to be in the range [0, num_labels-1]."
    ...
    def __getitem__(self, idx: int):
        ...
        try:        
            ...
            if self.point_drop > 0.0:
                ...
                coordinates, color, normals, segments, labels = coordinates[mask], color[mask], normals[mask], segments[mask], labels[mask]

            labels[:,0] = relabel_from_zero(labels[:,0])
            ...
            if self.load_grasps:
                ...
                try:
                    ...
                    print("Error indexing object grasps. Shapes:", grasp_label_info_object.shape, np.unique(labels[:,1].astype(int)).shape)
                    return None
                ...
                # if len(self.exclude_classes) > 0:
                #     # Remove this class from the labels
            ...
            if "train" in self.mode or "full" in self.mode or self.is_tta:
                ...
                if self.load_grasps:
                    grasp_contact_pts_scene = (transform[:3,:3] @ grasp_contact_pts_scene.T).T  + transform[:3, 3]
                    grasp_contact_pts_object = (transform[:3,:3] @ grasp_contact_pts_object.T).T  + transform[:3, 3]
                ...
                if augment:
                    ...
                    if sdf_query_points is not None:
                        sdf_query_points -= coordinates.mean(0)

                    try:
                        coordinates += np.random.uniform(coordinates.min(0), coordinates.max(0)) / 2
                        sdf_query_points += np.random.uniform(coordinates.min(0), coordinates.max(0)) / 2
                    ...
                    if self.flip_in_center:
                        assert False, "Flip in center is not supported for now"

                # for i in (0, 1):
                #     if random() < 0.5:
                #         coord_max = np.max(points[:, i])
                #         coordinates[:, i] = coord_max - coordinates[:, i]
                #         sdf_query_points[:, i] = coord_max - sdf_query_points[:, i]

                # TODO: DISABLED is_elastic_distortion for now
                # if random() < 0.95:
                ...
                if augment:
                    ...
                    pseudo_image = (255*color).astype(np.uint8)[np.newaxis, :, :]
                    color = np.squeeze(self.image_augmentations(image=pseudo_image)["image"])

                if self.point_per_cut != 0 and False: # TODO, fix this
                    ...
                    for _ in range(number_of_cuts):
                        ...
                        coordinates, normals, color, labels, segments = (
                            coordinates[~indexes],
                            normals[~indexes],
                            color[~indexes],
                            labels[~indexes],
                ...
                if augment and ((self.resample_points > 0) or (self.noise_rate > 0)):
            ...
            if not self.balance_sdf:
                ...
                sdf_mask_pos = np.random.choice(np.arange(len(sdf_query_points))[pos], self.num_sdf_points // 2, replace=pos.sum() < self.num_sdf_points // 2)
                sdf_mask_neg = np.random.choice(np.arange(len(sdf_query_points))[~pos], self.num_sdf_points - self.num_sdf_points // 2, replace=(~pos).sum() < self.num_sdf_points - self.num_sdf_points // 2)
                sdf_mask = np.concatenate((sdf_mask_pos, sdf_mask_neg))
                
            sdf_data = {
                "points": sdf_query_points[sdf_mask],
                "sdf_per_instance": sdf_instance_values[:len(instances)][..., sdf_mask] # ignore un-observed instances. This could mess with l1,l2 scores.
            } 
            ...
            # if self.balance_grasps:
            #     pos_scene = scene_centric_grasp_labels.max(-1)[..., -1] > 0
                
            #     if pos_scene.sum() > 0 and (~pos_scene).sum() > 0:
            #         grasp_mask_pos_scene = np.random.choice(np.arange(len(pos_scene))[pos_scene], self.num_grasp_points_scene // 2, replace=pos_scene.sum() < self.num_grasp_points_scene // 2)
            #         grasp_mask_neg_scene = np.random.choice(np.arange(len(pos_scene))[~pos_scene], self.num_grasp_points_scene - self.num_grasp_points_scene // 2, replace=(~pos_scene).sum() < self.num_grasp_points_scene - self.num_grasp_points_scene // 2)
            #         grasp_mask_scene = np.concatenate((grasp_mask_pos_scene, grasp_mask_neg_scene))
            #     else:
            #         grasp_mask_scene = np.random.choice(np.arange(len(scene_centric_grasp_labels)), self.num_grasp_points_scene)
            ...
            #     pos_object = object_centric_grasp_labels.max(-1)[..., -1] > 0

            #     if pos_object.sum() > 0 and (~pos_object).sum() > 0:
            #         grasp_mask_pos_object = np.random.choice(np.arange(len(pos_object))[pos_object], self.num_grasp_points_obj // 2, replace=pos_object.sum() < self.num_grasp_points_obj // 2)
            #         grasp_mask_neg_object = np.random.choice(np.arange(len(pos_object))[~pos_object], self.num_grasp_points_obj - self.num_grasp_points_obj // 2, replace=(~pos_object).sum() < self.num_grasp_points_obj - self.num_grasp_points_obj // 2)
            #         grasp_mask_object = np.concatenate((grasp_mask_pos_object, grasp_mask_neg_object))
            #     else:
            #         grasp_mask_object = np.random.choice(np.arange(len(object_centric_grasp_labels)), self.num_grasp_points_obj)

            #     grasp_contact_pts_object = grasp_contact_pts_object[grasp_mask_object]
            #     object_centric_grasp_labels = object_centric_grasp_labels[:, grasp_mask_object]
            ...
            print("Got error in dataloader! Scene: ",self.data[idx]["filepath"], e)
            raise e
            return None

        labels[:,0] = 0 # We only have one object class in this case.

        if self.dataset_name == "custom":
            return coordinates, features, labels, self.data[idx]['filepath'], \
                   raw_color, raw_normals, raw_coordinates, idx, sdf_data, transform, grasp_input, grasp_target, poses
        else:
            return coordinates, features, labels, self.data[idx]['raw_filepath'].split("/")[-2], \
                   raw_color, raw_normals, raw_coordinates, idx, transform
