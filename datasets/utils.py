...
class VoxelizeCollateSDF:
    ...
    def __call__(self, batch):
        ...
        if ("train" in self.mode) and self.very_small_crops:
            batch = make_crops(batch)
        return voxelizeSDF(batch, self.ignore_label, self.voxel_size, self.probing, self.mode,
                        task=self.task, ignore_class_threshold=self.ignore_class_threshold,
                        filter_out_classes=self.filter_out_classes, label_offset=self.label_offset,
                        num_queries=self.num_queries)
...
def voxelize(batch, ignore_label, voxel_size, probing, mode, task,
    ...
    (coordinates, features, labels, original_labels, inverse_maps, original_colors, original_normals,
     original_coordinates, idx, transforms) = (
        [],
        [],
        [],
        [],
    ...
    for sample in batch:
        ...
        if len(sample) >= 9:
            ...
            sdf_values.append(sdf_values)
        transforms.append(sample[9])

            # print("sample in collate", sample[8])
    ...
    if "train" not in mode:
        return (
            ...
            [sample[3] for sample in batch], transforms
        )
    else:
        return (
...
#                 raw_color, raw_normals, raw_coordinates, idx, sdf_data, transform, grasp_input, grasp_target


def voxelizeSDF(batch, ignore_label, voxel_size, probing, mode, task,
             ignore_class_threshold, filter_out_classes, label_offset, num_queries):
    (coordinates, features, labels, original_labels, inverse_maps, original_colors, original_normals,
     original_coordinates, idx, sdf_query_coords, sdf_values, transforms, 
     poses) = (
        [],
        [],
        [],
    ...
    for sample in batch:
        ...
        if grasp_input is not None:
            obj_grasp_pts.append(torch.from_numpy(grasp_input["grasp_points_object"]).float())
            scene_grasp_pts.append(torch.from_numpy(grasp_input["grasp_points_scene"]).float())

        if grasp_target is not None:
            obj_grasp_labels.append(torch.from_numpy(grasp_target["object_centric_labels"]).float())
            scene_grasp_labels.append(torch.from_numpy(grasp_target["scene_centric_labels"]).float())
    ...
    grasp_points_object = None# torch.stack(obj_grasp_pts_sub)
    grasp_labels_object = None

    grasp_points_scene = None# torch.stack(scene_grasp_pts)
    grasp_labels_scene = None
    ...
    if grasp_input is not None:
        ...
        grasp_points_object = obj_grasp_pts_sub# torch.stack(obj_grasp_pts_sub)
        grasp_labels_object = obj_grasp_labels_sub

        grasp_points_scene = scene_grasp_pts# torch.stack(scene_grasp_pts)
    ...
    for idx in range(len(target)):
        ...
        if grasp_labels_object is not None and len(grasp_labels_object) > 0:
            target[idx]["object_centric_labels"] = grasp_labels_object[idx]
        if grasp_labels_scene is not None and len(grasp_labels_scene) > 0:
            target[idx]["scene_centric_labels"] = grasp_labels_scene[idx]
        
    if "train" not in mode:
        return (
            NoGpu(coordinates, features, original_labels, inverse_maps,
                  target_full, original_colors, original_normals, original_coordinates, idx, full_res_coords = full_res_coords, sdf_queries=sdf_query_coords, full_res_features = full_res_features, sdf_values=sdf_values,
                   grasp_points_object=grasp_points_object, grasp_points_scene=grasp_points_scene, poses=poses), target,
            [sample[3] for sample in batch], transforms
        )
    else:
        return (
            NoGpu(coordinates, features, original_labels, inverse_maps, full_res_coords = full_res_coords, sdf_queries=sdf_query_coords, full_res_features=full_res_features, sdf_values=sdf_values, 
            grasp_points_object=grasp_points_object, grasp_points_scene=grasp_points_scene, poses=poses), target,
            [sample[3] for sample in batch], transforms
        )
...
class NoGpu:
    def __init__(
            ...
            idx=None, sdf_queries=None, sdf_values=None, 
            grasp_points_object = None, grasp_points_scene = None, 
            object_centric_labels = None, scene_centric_labels = None, poses = None, full_res_features = None,full_res_coords=None, 
    ):
