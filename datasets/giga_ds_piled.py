...
def relabel_from_zero(array: np.ndarray):
    return np.unique(array, return_counts=True, return_inverse=True)[1]
...
class SemanticSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.load_grasps:
            ...
            try:
                if (len(np.unique(labels[:,1].astype(int))) + 1) == grasp_label_info_object.shape[0]:
                    object_centric_grasp_labels = grasp_label_info_object[1:]  #[0]
                else:
                    object_centric_grasp_labels = grasp_label_info_object[np.unique(labels[:,1].astype(int))- 1]  #[0]
            except:
                ...
                object_centric_grasp_labels =  None
            
            try:
                if (len(np.unique(labels[:,1].astype(int))) + 1) == grasp_label_info_scene.shape[0]:
                    scene_centric_grasp_labels = grasp_label_info_scene[1:]  #[0]
                else:
                    scene_centric_grasp_labels = grasp_label_info_scene[np.unique(labels[:,1].astype(int))- 1]  #[0]
      
            except:
                print("Error indexing scene grasps. Shapes:", grasp_label_info_scene.shape, np.unique(labels[:,1].astype(int)).shape, "instances", np.unique(labels[:,1].astype(int)))
                return None
        ...
        # Remap labels
        labels[:, 1] = relabel_from_zero(labels[:, 1])
        ...
        if len(instances) != len(sdf_instance_values):
            ...
            try:
                sdf_instance_values = sdf_instance_values[
                    np.unique(instances) - len(self.exclude_classes) - 1
                ]
        ...
        if self.balance_grasps and self.load_grasps:
            ...
            if object_centric_grasp_labels is not None:
