...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        for d in self._data:
            ...
            file_name = os.path.basename(d["filepath"]).replace(".npy", "")
            dir_name = os.path.dirname(d["filepath"])
            obj_qual_exists =  os.path.exists(os.path.join(dir_name, "grasps", file_name + "_object_quality.npy"))
            scene_qual_exists =  os.path.exists(os.path.join(dir_name, "grasps", file_name + "_scene_quality.npy"))

            if os.path.exists(d["filepath"]):
                if not scene_qual_exists or not obj_qual_exists:
                    print("labels missing for", obj_qual_exists, scene_qual_exists)
                else:
                    ddata.append(d)
            else:
                print("skipping entry:", d["filepath"], "since it does not exist!")
    ...
    def __getitem__(self, idx: int):
        ...
        if self.load_grasps:
            ...
            grasp_query_info_object = np.load(
                os.path.join(dir_name, "grasps", file_name + "_object_query.npy")
            )
            grasp_query_info_scene = np.load(
                os.path.join(dir_name, "grasps", file_name + "_scene_query.npy")
            )

            grasp_label_info_object = (
                np.load(
                    os.path.join(dir_name, "grasps", file_name + "_object_quality.npy")
                )
                > 0
            )
            grasp_label_info_scene = (
                np.load(
                    os.path.join(dir_name, "grasps", file_name + "_scene_quality.npy")
                )
                > 0
            )

            # shape of grasp labels is (2, num_instances, num_grasps, num_labels)
            object_centric_grasp_labels = grasp_label_info_object
            # [
            #     np.unique(labels[:, 1].astype(int))
            # ]  # [0]

            scene_centric_grasp_labels = grasp_label_info_scene
            # [
            #     np.unique(labels[:, 1].astype(int))
            # ]  # [1]
            ...
            grasp_contact_pts_object = (
                grasp_query_info_object
                if not self.scene_based
                else grasp_query_info_scene
            )
            ...
            if 0 not in self.exclude_classes:  # we do want to load the table class
                ...
                grasp_contact_pts_scene = np.concatenate(
                    [grasp_contact_pts_scene, table_points], axis=0
                )
                grasp_contact_pts_object = np.concatenate(
                    [grasp_contact_pts_object, table_points], axis=0
                )
                ...
                object_centric_grasp_labels = np.concatenate(
                    [object_centric_grasp_labels, pseudo_grasps], axis=1
                )
                scene_centric_grasp_labels = np.concatenate(
                    [scene_centric_grasp_labels, pseudo_grasps], axis=1
                )
        ...
        if self.balance_grasps and self.load_grasps:
            pos_scene = scene_centric_grasp_labels.max(-1).max(0) > 0
            ...
            pos_object = object_centric_grasp_labels.max(-1).max(0) > 0
