...
class CustomPreprocessing(BasePreprocessing):
    ...
    def process_file(self, filepath, mode):
        ...
        if ds_root not in self.grasps:
            self.grasps[ds_root] = pd.read_csv(
                os.path.join(ds_root, "grasps_candidate.csv")
            )
        grasps_df = self.grasps[ds_root]
        print(parent_dir)
        ...
        if not os.path.exists(self.save_dir / mode / "sdf" / scene_name):
        ...
        if num_sdf_instances != num_mesh_instances:
            print(
                "num_instances {} != num_mesh_instances {}".format(
                    num_sdf_instances, num_mesh_instances
                )
            )
            print("Skipping entry", self.save_dir / mode / "sdf" / scene_name)
            return None
        # save_sdf()
        ...
        # if not os.path.exists(self.save_dir / mode / "grasps" / f"{scene_name}_object_query.npy"):
        ...
        if not (instances == 1).any():  # Weird behaviour. Instances are somehow shifted
        ...
        if len(np.unique(instances)) != num_sdf_instances:  # table is counted
            print(
                filepath,
        ...
        grasps = grasps_df[grasps_df["scene_id"] == os.path.basename(filepath)[:-4]]
        ...
        if not os.path.exists(
            ...
            x_axis = grasps[["ax_x", "ax_y", "ax_z"]].to_numpy().astype(np.float32)
            y_axis = grasps[["ay_x", "ay_y", "ay_z"]].to_numpy().astype(np.float32)
            z_axis = grasps[["az_x", "az_y", "az_z"]].to_numpy().astype(np.float32)
            orientations = np.stack([x_axis, y_axis, z_axis], axis=-1)
            contacts = grasps[["x", "y", "z"]].to_numpy().astype(np.float32)
            widths = grasps["width"].to_numpy().astype(np.float32)
            labels = grasps[[f"label_{i}" for i in range(12)]].to_numpy()

            target = grasps["target"].to_numpy().astype(np.int32)
            object_centric_labels = np.zeros((num_sdf_instances, len(grasps), 12))
            scene_centric_labels = np.zeros((num_sdf_instances, len(grasps), 12))
            for instance_id in np.unique(target):
                try:
                    ...
                    object_centric_labels[
                        instance_id, target_mask
                    ] = object_centric_succ
                    scene_centric_labels[instance_id, target_mask] = target_labels == 1
                    object_centric_query_pts = contacts
                except:
                    print(
                        "[ERROR] getting object centric labels for instance",
                        instance_id,
                        "in scene",
                        scene_name,
                    )
            ...
            if self.grasp_store_data:
                ...
                object_centric_labels = np.zeros(
                    (num_sdf_instances, sum(point_indices), 12)
                )
            ...
            np.save(
                processes_grasps_filepath_qual, scene_centric_labels.astype(np.int32)
            )
            np.save(
                processes_grasps_filepath_qual_object,
                object_centric_labels.astype(np.int32),
            )
        ...
        if not os.path.exists(processed_gt_filepath):
        ...
        if False:  # Visualize
            ...
            instances = o3d.geometry.PointCloud()
            instances.points = o3d.utility.Vector3dVector(pts)
            import matplotlib.pyplot as plt

            colmap = plt.get_cmap("tab10")
            print(np.unique(points[:, -1]))
            instances.colors = o3d.utility.Vector3dVector(
                colmap(points[:, -1] / 10)[:, :3]
            )
            visdata = [cloud, instances]
            vis.add_pointcloud(cloud)
            for instance_id, sdf in enumerate(sdf_file["sdf"]):
                # sdf = sdf[sdf > 0]
                c = o3d.geometry.PointCloud()
                c.points = o3d.utility.Vector3dVector(sdf_file["points"][sdf > 0])
                visdata.append(c)
                lbls = object_centric_labels.max(axis=-1)[instance_id]
                colors = colmap(lbls)[:, :3]

                pc = o3d.geometry.PointCloud()
                pc.points = o3d.utility.Vector3dVector(object_centric_query_pts)
                pc.colors = o3d.utility.Vector3dVector(
                    np.array([1, 0, 0])[None, ...] * lbls[..., None] > 0
                )
                visdata.append(pc)
                # Add Grasp points
                # object
                # object_centric_query_pts
            o3d.visualization.draw(visdata)
            ...
            # orients = (
            #     Rotation.from_matrix(orientations.astype(np.float)).inv().as_quat()
            # )
            # contacts = contacts.astype(np.float)
            # vis.add_grasps(list(zip(contacts, orients)), convention="ours")
            # pdb.set_trace()
            # object = True
